...
mu1 = q.mu;
k = length(mu0);
sigma0 = p.sigma;% + 1e-6*eye(size(p.sigma,1));%trick to avoid singularities
sigma1 = q.sigma;% + 1e-6*eye(size(p.sigma,1));

%TODO suboptimale implementierung durch meine eigene ersetzen
